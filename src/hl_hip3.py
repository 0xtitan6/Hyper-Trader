...
def register_hip3_dexes(info: InfoProto, market_meta: Any | None = None) -> int:
    ...
    for i, dex_info in enumerate(dexes[1:]):
        ...
        # Push szDecimals into the rounder. Must happen for BOTH the
        # set_perp_meta path and the AttributeError fallback above — the SDK
        # maps are keyed by asset_id, MarketMeta is keyed by coin name, and
        # only the latter is consulted by round_size/round_price.
